 ...
 */
public class AdminCandidateJdbcDao extends
		GenericJdbcDao<AdminCandidate, Integer> implements AdminCandidateDao
{
	...
	public AdminCandidate getAdminCandidateByKey(String key) {
		...
		String query = "SELECT * FROM " + table + " WHERE access_key = ? AND "
	...
	public AdminCandidate getAdminCandidateByEmail(String email) {
		AdminCandidate adminCandidate = null;
		String query = "SELECT * FROM " + table + " WHERE email = ? AND "
				+ GenericDao.NOT_DELETED;
		try (Connection connection = connectionPool.getConnection();
				PreparedStatement statement = connection
						.prepareStatement(query)) {
	...
	public List<AdminCandidate> getAdminCandidateByExpireDate(
			Timestamp timestamp)
	{
		List<AdminCandidate> adminCandidates = new ArrayList<>();
		String query = "SELECT * FROM " + table + " WHERE expire = ? AND "
				+ GenericDao.NOT_DELETED;
		try (Connection connection = connectionPool.getConnection();
				PreparedStatement statement = connection
						.prepareStatement(query)) {
