 ...
 */
public class FeedbackJdbcDao extends GenericJdbcDao<Feedback, Integer>
		implements FeedbackDao
{
	...
	public List<Feedback> getFeedbacksByCustIdForHim(Integer id) {
		String query = "SELECT * FROM " + table
				+ " WHERE cust_id = ? AND author = 'dev' AND "
				+ GenericDao.NOT_DELETED;
		return getFeedbacksByQuery(query, id);
	}
	...
	public List<Feedback> getFeedbacksByDevIdForHim(Integer id) {
		String query = "SELECT * FROM " + table
				+ " WHERE dev_id = ? AND author = 'customer' AND "
				+ GenericDao.NOT_DELETED;
		return getFeedbacksByQuery(query, id);
	}
	...
	public int deleteDevFeed(Integer devId, Integer feedId) {
		...
		try (Connection connection = connectionPool.getConnection();
				PreparedStatement statement = connection
						.prepareStatement("delete from feedback where dev_id = ? && id = ? AND "
								+ GenericDao.NOT_DELETED)) {
	...
	public int deleteCustFeed(Integer custId, Integer feedId) {
		...
		try (Connection connection = connectionPool.getConnection();
				PreparedStatement statement = connection
						.prepareStatement("delete from feedback where cust_id = ? && id = ? AND "
								+ GenericDao.NOT_DELETED)) {
	...
	public Integer getAvgRate(Integer dev_id) {
		...
		try (Connection connection = connectionPool.getConnection();
				PreparedStatement statement = connection
						.prepareStatement("SELECT AVG(rate) from feedback WHERE dev_id = ? AND "
								+ GenericDao.NOT_DELETED)) {
